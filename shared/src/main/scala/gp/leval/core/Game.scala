...
case class Game(
    ...
    being.face match {
      case Card(Rank.King, _) | Card(Rank.Jack, Suit.Heart) =>
        beings.find {
          ...
          (g1.directEffect(15, b.owner), Some(c))
        } getOrElse ((g1, None))
      case _ => (g1, None)
    }
  ...
  def burry(target: Card, cards: List[Card]): Game =
    copy(beings = beings - target, deathRiver = cards reverse_::: deathRiver)
  ...
  def attackBeing(
    ...
    val (g0, removed0) = rules
      .onAttack(this, origin, target)
      ...
      if removed0 then (g0, removed0)
        ...
        (
          (if removeCard then
           ...
           else g0).copy(beingsState = globalNewState),
          removeCard
        )
      }
  ...
  def educate(e: Educate): Game = {
    ...
    copy(
      beings = beings + (newBeing.face -> newBeing),
      stars = stars.set(b.owner, newStar)
    )
  }

  def revealAndLookLoverCheck(
      targetfc: Card,
      s: Suit,
      sAttacker: Option[CardOrigin],
      removeGard: Int => Boolean
  ): (Game, Boolean) = {
  ...
  def lookCard(o: CardOrigin, targetfc: Card, s: Suit): (Game, Boolean) = {
    ...
    (g.copy(lookedCards = lookedCards + ((targetfc, s))), cardRemoved)
  }
  ...
  def revealCard(
    ...
    val (g, cardRemoved) =
      revealAndLookLoverCheck(targetfc, s, sAttacker, _ => true)
    (g.copy(revealedCard = revealedCard + ((targetfc, s))), cardRemoved)
  }
...
open class MutableGame(var game: Game) /*extends (Move ~> Id)*/ {
  ...
  def apply[A](ma: Move[A]): A /*Id[A]*/ = ma match {
    ...
    case AttackBeing(attack, target, targetedSuit) =>
      val (g, toBury, numToCollect) =
        game.attackBeing(attack, target, targetedSuit)
      game = g
      (toBury, numToCollect)
