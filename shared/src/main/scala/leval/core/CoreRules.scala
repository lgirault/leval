  ...
  def drawAndLookValues(origin: Origin): (Int, Int) =
    origin match {
      ...
      case co @ CardOrigin.Being(_, _) =>
        throw new Exception(s"drawAndLookValues ${co} should not happen")
    }
  ...
  def result(g: Game): Option[(PlayerId, PlayerId)] =
    if (g.source.isEmpty) None
      ...
      if (result.nonEmpty) result
        ...
        someLoser map { case (s, i) =>
          (g.stars((i + 1) % 2).id, s.id)
        }
      }
    }
...
case object Helios extends AntaresHeliosCommon with Serializable {
  val id = "helios"
